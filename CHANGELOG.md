...
- `l9_sanity` is no longer suppressed on a zero count. `AppendCountSanity` returned early whenever the matched count was zero, so the most suspicious outcome was the one case that got no guardrail. A zero count from a pipeline containing a `parse` stage now carries a note pointing at `sample_body` to confirm the real Body shape before retrying. A zero count with no `parse` stage is still left untouched, and non-zero behaviour is unchanged.
