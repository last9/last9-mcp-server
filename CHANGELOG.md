...
- HTTP mode (`--http`) now attributes each tool call to the request that made it. The Streamable HTTP handler was built with the uninstrumented upstream constructor, which left the server transport unset, so every concurrent client shared a single `unknown_client` bucket and traces from unrelated sessions were grouped into one. The handler now comes from the instrumented `last9/mcp-go-sdk` v0.1.4 helper, which records `mcp.server.transport=streamable` and gives anonymous stateless requests isolated per-request client IDs. This also bumps `modelcontextprotocol/go-sdk` to v1.7.0, required by that SDK release. The handler stays stateless and the tool and prompt surfaces are unchanged.
