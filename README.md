...
- `LAST9_DEBUG_CHUNKING`: Set to `true` to emit chunk-planning and per-chunk execution logs for `get_logs` and `get_service_logs`. This also maps to the `-debug_chunking` CLI flag.
