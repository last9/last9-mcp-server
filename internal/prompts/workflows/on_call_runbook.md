...
{{else}}Call get_service_profile(service_name={{.service}}) before symptom routing. Use the result for all routing below — do not re-derive telemetry shape via PromQL or attribute probing.{{if not .env}} Pick env from profile.deployment.envs; if multiple or empty, call get_service_environments.{{end}}
