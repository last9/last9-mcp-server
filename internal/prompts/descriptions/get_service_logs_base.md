...
**Use this tool when (outside `get_exceptions` investigation flows):** filtering by log severity levels (`severity_filters`: error/warn/fatal) or plain-text message search (`body_filters`) for a single known service—no pipeline needed.
