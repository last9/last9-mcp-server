...
- `get_log_attributes_for_pipeline` now reports the Body's actual shape when it has no recognized structure. Previously a plain-text Body (neither JSON nor logfmt, no severity token) produced no Body-derived entry at all, leaving only the bare `{"$eq":["Body","<value>"]}` catalog hint — so models guessed `parser:"json"`, matched nothing, and returned a silent zero, or wrote an unanchored regexp capture that matched a leading timestamp and returned a confidently wrong count. Such services now get a single `Body` entry carrying `source:"body"`, up to three `sample_bodies` example lines, and a one-stage `$regex`-on-Body hint to anchor against. `sample_bodies` preserves the line's byte shape (whitespace runs intact, credential keys kept, newlines escaped visibly) so a regexp can be anchored to it; it redacts URLs and recognized credential values only and is **not** PII-redacted, so treat it as untrusted log content. A `sample_notes` list reports which transforms fired on the returned samples. The plaintext entry also survives the indexed/body merge: it enriches the existing indexed `Body` entry instead of being dropped as a duplicate `filter_field`.
- `l9_sanity` is no longer suppressed on a zero count. `AppendCountSanity` returned early whenever the matched count was zero, so the most suspicious outcome was the one case that got no guardrail. A zero count from a pipeline that **parses or filters `Body`** now carries a note pointing at `sample_bodies` to confirm the real Body shape before retrying — this deliberately covers the parse-free `$regex`-on-Body shape that `get_log_attributes_for_pipeline` itself recommends, since a pattern copied verbatim is valid regexp that matches nothing. The note also warns that an unanchored capture group can match the wrong token and return a confidently wrong *nonzero* count. A new `service_log_volume` key in the block carries the service's total log count for the window, distinguishing a genuine zero (service emitted nothing) from a parse/filter mismatch. A zero count from a pipeline that neither parses nor touches `Body` is still left untouched, and non-zero behaviour is unchanged.
