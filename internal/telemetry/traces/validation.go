...
var standardTraceFields = map[string]struct{}{
	"TraceId":       {},
	"SpanId":        {},
	"ServiceName":   {},
	"SpanName":      {},
...
func validateTraceJSONQuery(pipeline []interface{}, attributes []string) error {
	fields, derived := collectTraceFieldsFromPipeline(pipeline)
	...
	for field := range fields {
		...
		if _, ok := standardTraceFields[field]; ok {
			continue
		}
		if len(allowed) == 0 {
		...
		if isAllowedTraceAttributeField(field, allowed) {
			continue
		}
		invalid = append(invalid, field)
	}
	...
	return fmt.Errorf("trace query uses unsupported fields: %s", strings.Join(invalid, ", "))
}

func collectTraceFieldsFromPipeline(pipeline []interface{}) (map[string]struct{}, map[string]struct{}) {
...
func isAllowedTraceAttributeField(field string, allowed map[string]struct{}) bool {
