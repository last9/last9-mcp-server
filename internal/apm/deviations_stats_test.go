...
func TestDeviationWindowResolverAvailableOnAllPlatforms(t *testing.T) {
	now := time.Date(2026, 7, 11, 10, 7, 32, 0, time.UTC)
	if _, err := resolveDeviationWindows(DeviationArgs{}, now, time.Minute); err != nil {
		t.Fatal(err)
	}
}
...
func TestResolveDeviationWindowsPreviousPeriod(t *testing.T) {
	...
	if got.EffectiveCurrentStart.Before(got.RequestedCurrentStart) {
		t.Fatalf("effective start moved earlier than requested: %+v", got)
	}
}
...
func TestResolveDeviationWindowsRelativeLookback(t *testing.T) {
	...
	if got.EffectiveCurrentStart != time.Date(2026, 7, 11, 9, 53, 0, 0, time.UTC) ||
		got.EffectiveCurrentEnd != time.Date(2026, 7, 11, 10, 7, 0, 0, time.UTC) {
...
func TestResolveDeviationWindowsRejectsUnalignedHistoricalBoundaries(t *testing.T) {
	now := time.Date(2026, 7, 11, 10, 7, 32, 0, time.UTC)
	for _, args := range []DeviationArgs{
		{StartTimeISO: "2026-07-11T07:00:30Z", EndTimeISO: "2026-07-11T08:00:30Z"},
		{
			StartTimeISO:     "2026-07-11T07:00:00Z",
			EndTimeISO:       "2026-07-11T08:00:00Z",
			BaselineStartISO: "2026-07-10T07:00:30Z",
			BaselineEndISO:   "2026-07-10T08:00:30Z",
		},
	} {
		if _, err := resolveDeviationWindows(args, now, time.Minute); err == nil {
			t.Fatalf("expected unaligned historical boundary error for %+v", args)
		}
	}
}
...
func TestResolveDeviationWindowsAlignsBothWindowsToCompletedBuckets(t *testing.T) {
	...
	if got.EffectiveCurrentStart != time.Date(2026, 7, 11, 9, 58, 0, 0, time.UTC) ||
		got.EffectiveCurrentEnd != time.Date(2026, 7, 11, 10, 7, 0, 0, time.UTC) {
		t.Fatalf("unexpected effective current window: %+v", got)
	}
