...
Reading `evidence.warnings`: `candidate_limit_reached` on a discovery call means discovery filled its server-side candidate budget, not that the request was wrong — pass explicit `candidate_attributes` to examine dimensions it did not reach. If the call fails with the capability being unavailable or disabled, this analysis is not enabled for the connected tenant; do not retry, and fall back to `get_traces` aggregations.
