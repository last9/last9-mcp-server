...
**l9_sanity:** high `ratio` or "filter likely too broad" → next call `get_logs` with `aggregate`+`$count` and an ERROR/`SeverityText` gate. `matched_count: 0` (no `ratio`): genuine zero or parse-stage mismatch; check `sample_body` via get_log_attributes_for_pipeline, don't narrow.
