...
{{if not .service}}No service given, so start with fleet triage: call get_alerts to find firing alerts, then get_service_summary to spot the unhealthy services, and scope the branch below to the worst offender. Once the target service is identified, call get_service_profile(service_name=<service>) before symptom routing.
...
{{else if eq .symptom "errors"}}Errors: get_service_performance_details, then get_exceptions, then get_service_traces, then aggregate get_logs (count by logger; use profile.level_field when severity_set is none or partial; gate on ERROR/FATAL) before reading raw lines.
