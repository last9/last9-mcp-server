...
3. If telemetry.traces == "present", call get_service_traces for representative slow traces of the deviating operation; otherwise skip trace tools.
4. Call get_service_dependency_graph to see whether the latency originates downstream.
5. If a downstream span errors and telemetry.traces == "present", call get_exceptions on that service to confirm the root cause.
