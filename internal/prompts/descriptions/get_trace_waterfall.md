...
Parameters:
- `trace_id` (required): exact 32-character hexadecimal OpenTelemetry trace ID.
...
That last one means the span you asked for was not in the returned set — a wrong ID, or a span removed by truncation, the environment filter, or an unparseable timestamp. Do not retry with the same ID; widen `max_spans` or read `data.spans` for the IDs that were returned.
