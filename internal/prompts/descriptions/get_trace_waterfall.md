...
The response is an `investigation-evidence/v1` envelope: `request` echoes the scope and half-open windows, `evidence` carries `partial`, `truncated`, `warnings`, and `provenance`, `interpretation` carries `evidence_quality` and `claim_type`, and the waterfall itself is under `data`. Because a trace-ID lookup is not service-scoped, `request.scope.service_name` is derived from the root span and `request.scope.environment` is `unspecified` when the `environment` argument is omitted.
