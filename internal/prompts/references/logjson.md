...
Do not invent org-specific attribute names. Discover available fields with `get_log_attributes` / `get_log_attributes_for_pipeline` before filtering. If a field matches `resource_fieldname`, use `resources['fieldname']`; otherwise `attributes['fieldname']`.

If a needed field is not returned by discovery tools, fall back to a regex-based filter/parser instead of inventing attribute names.
