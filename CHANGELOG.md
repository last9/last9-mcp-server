...
- `get_apm_service_deviations` now over-fetches its traffic-based candidate mask (`topk(3 × limit, …)` instead of `topk(limit, …)`) for both service and operation scope, letting the magnitude-aware in-process selection make the final cut. Previously the mask returned exactly `max_services` identities selected purely by traffic volume, so a low-traffic service with a high-magnitude regression could be excluded before any deviation math ran — the condition the magnitude-priority cap fix exists to handle, which was otherwise unreachable through a datasource whose `topk` returns exactly k series. Over-fetching bounds query cost but is a recall improvement, not a completeness guarantee.
