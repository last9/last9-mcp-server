...
2. {{if not .db_system}}From step 1, pick the database system behind the slowest queries, then call {{else}}Call {{end}}get_database_server_metrics ({{if .db_system}}db_system={{.db_system}}{{else}}db_system = the system identified in step 1{{end}}) over {{.time}} to check whether server-side pressure (connections, memory, disk) explains the slowness.
