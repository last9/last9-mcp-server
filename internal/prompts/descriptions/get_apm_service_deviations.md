...
- Treat `stable`, `no_data`, and `unsupported_workload_shape` as terminal comparison outcomes. Answer from that result and do not automatically call follow-up tools unless the user explicitly requested a deeper investigation. Terminal outcomes also include an explicit `warnings` line in the tool result repeating this stop rule.
